...
def det_hehe(ded):
    return ded[1:] == [0, 0, 0, 1]

def det_nigga(ded, lm_mano, lm_cara):
    if ded[1:] != [1, 0, 0, 0]:
        return False
    return min(d(lm_mano[8], lm_cara[33]), d(lm_mano[8], lm_cara[263])) < 0.15
...
def main():
    ...
    cv2.moveWindow("Tu Camara", 100, 100)
    cv2.moveWindow("Meme Detectado", 800, 100)
    ...
    while True:
        ...
        if lm_cara and len(manos) == 2 and det_sonic(manos, lm_cara):
            ...
            if det_rata(ded_m):
                ...
                det = "hehe.jfif"
            elif lm_cara and det_nigga(ded_m, lm_m, lm_cara):
                det = "nigga.png"
